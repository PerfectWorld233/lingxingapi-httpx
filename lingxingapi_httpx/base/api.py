...
from lingxingapi_httpx import utils, errors
from lingxingapi_httpx.base import route, schema
...
class BaseAPI:
    ...
    async def _request(
        ...
        while True:
            # fmt: off
            # 确保 HTTP 会话可用
            if BaseAPI._session is None or BaseAPI._session.is_closed:
                BaseAPI._session = httpx.AsyncClient(
                    base_url=route.API_SERVER,
                    headers={"Content-Type": "application/json"},
                    timeout=Timeout(self._timeout),
                    limits=Limits(max_connections=self._max_tcp_connections),
                )
            # 发送请求
            try:
                res = await BaseAPI._session.request(
                    method,
                    url,
                    params=params,
                    json=body,
                ...
                if res.status_code != 200:
                    if res.status_code in (501, 502, 504):
                        raise errors.InternalServerError("领星API服务器内部错误", url, res.reason_phrase, res.status_code)
                    raise errors.ServerError("领星API服务器响应错误", url, res.reason_phrase, res.status_code)
                # . 解析并验证响应数据
                return self._handle_response_data(
                    url, res.content, extract_data
                )
            # 请求限流错误处理
            ...
            # httpx 超时异常映射
                ...
                if not await utils.check_internet_tcp():
                    if (
                        ...
                        continue
                    exc = errors.InternetConnectionError("无法链接互联网, 请检查网络连接", url, str(mapped))
                    if params is not None:
                ...
                if (
                    ...
                    logger.log(
                        ...
                        self._ignore_api_limit_wait, retry_count, mapped,
                        stacklevel=3
                    )
                    await _aio_sleep(self._ignore_api_limit_wait)
                    continue
                exc = errors.ApiTimeoutError("领星API请求超时或错误, 若无网络问题, 请检查IP白名单设置", url, str(mapped))
                if params is not None:
                    exc.add_note("请求参数: %r" % params)
                if body is not None:
                    exc.add_note("请求实体: %r" % body)
            ...
            # httpx 连接/网络错误映射
            except (httpx.ConnectError, httpx.NetworkError, httpx.HTTPError) as err:
                mapped = errors.ClientConnectionError(str(err))
                if not await utils.check_internet_tcp():
                    if (
                        self._ignore_internet_connection
                        and (self._infinite_internet_connection_retry or retry_count < self._ignore_internet_connection_retry)
            ...
            # 网络相关错误处理 (原生 asyncio/aiohttp 异常)
